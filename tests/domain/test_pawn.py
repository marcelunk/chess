...
from chess.domain.rules.move_validator import _get_legal_moves_pawn
...
def test_possible_moves_white_from_start_position():
    ...
    moves = _get_legal_moves_pawn(source, game_state, pawn)
    assert len(moves) == 2
...
def test_possible_hits_white_from_start_position():
    ...
    moves = _get_legal_moves_pawn(Square.from_string(position_pawn), game_state, pawn)
    assert len(moves) == 3
    assert Square.from_string('b3') in moves
...
def test_possible_hits_white():
    ...
    moves = _get_legal_moves_pawn(Square.from_string('d4'), game_state, pawn)
    assert len(moves) == 2
...
def test_en_passant_white_pawn():
    ...
    moves = _get_legal_moves_pawn(Square.from_string('c5'), next_game_state, white_pawn)
    assert len(moves) == 2
    assert Square.from_string('b6') in moves
    assert Square.from_string('c6') in moves
...
def test_en_passant_black_pawn():
    ...
    moves = _get_legal_moves_pawn(Square.from_string('b4'), next_game_state, black_pawn)
    assert len(moves) == 2
    assert Square.from_string('b3') in moves
    assert Square.from_string('c3') in moves
