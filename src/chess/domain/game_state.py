...
class GameStateFactory:
    ...
    def _init_empty_board():
        piece_by_square = dict()
        for file in range(0, 8):
            for rank in range(0, 8):
                piece_by_square[Square(file, rank)] = None

        return piece_by_square

    @staticmethod
    def _init_pieces(piece_by_square):
        white_pawns = GameStateFactory._get_pieces(Pawn, 8, Color.WHITE)
        for i in range(0, 8):
            GameStateFactory._init_square(Square(i, 1), white_pawns[i], piece_by_square)
        white_rooks = GameStateFactory._get_pieces(Rook, 2, Color.WHITE)
        ...
        white_knights = GameStateFactory._get_pieces(Knight, 2, Color.WHITE)
        ...
        white_bishops = GameStateFactory._get_pieces(Bishop, 2, Color.WHITE)
        ...
        black_pawns = GameStateFactory._get_pieces(Pawn, 8, Color.BLACK)
        for i in range(0, 8):
            GameStateFactory._init_square(Square(i, 6), black_pawns[i], piece_by_square)
        black_rooks = GameStateFactory._get_pieces(Rook, 2, Color.BLACK)
        ...
        black_knights = GameStateFactory._get_pieces(Knight, 2, Color.BLACK)
        ...
        black_bishops = GameStateFactory._get_pieces(Bishop, 2, Color.BLACK)
    ...
    @staticmethod
    def _get_pieces(type, amount, color):
        pieces = list()
        count = 0
        while count < amount:
